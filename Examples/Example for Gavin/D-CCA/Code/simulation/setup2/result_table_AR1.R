...
for(i in 1:4){
  for(j in 1:4){
    ...
    for(k in 1:1000){
      data[k,]=as.matrix( read.table(paste('siml2_AR1_theta_ED_seed',k,'_p',p[i],'_angle',theta[j],'_nstd1.txt',sep='') ))[,1]
    }
...
for(i in 1:4){
  for(j in 1:4){
    data=matrix(0,1000,2)
    for(k in 1:1000){
      data[k,]=as.matrix( read.table(paste('siml2_AR1_theta_ED_seed',k,'_p900_angle',theta[j],'_nstd',sigma[i],'.txt',sep='') ))[,1]
    }
