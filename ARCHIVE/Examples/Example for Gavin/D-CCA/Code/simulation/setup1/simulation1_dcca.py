        ...
        for std_noise in std_noise_list[change]:
            ...
            for angle_deg in angle_deg_list[change]:         
               ...
               # V_2 = ortho_group.rvs(dim=p_2,random_state=0)[:,:r_2]  
                ...
                np.savetxt('siml1_seed' + sys.argv[1] + '_p' + str(p_1) + '_angle' + str(angle_deg) + '_nstd' + str(std_noise)  +'.txt', np.array([out1, out1_rank, out2, out2_rank, out3, out3_rank, out4, out5]), delimiter=' ')
                
                np.savetxt('siml1_theta_Oracle_seed' + sys.argv[1] + '_p' + str(p_1) + '_angle' + str(angle_deg) + '_nstd' + str(std_noise)  +'.txt', np.array([ccor_Oracle, theta_Oracle]), delimiter=' ')
                np.savetxt('siml1_theta_ED_seed' + sys.argv[1] + '_p' + str(p_1) + '_angle' + str(angle_deg) + '_nstd' + str(std_noise)  +'.txt', np.array([ccor_ED, theta_ED]), delimiter=' ')
                np.savetxt('siml1_theta_GR_seed' + sys.argv[1] + '_p' + str(p_1) + '_angle' + str(angle_deg) + '_nstd' + str(std_noise)  +'.txt', np.array([ccor_GR, theta_GR]), delimiter=' ')
