...
OnPLS=NULL
for(seed in 1:1000){
  DCCA=rbind(as.numeric(as.matrix(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1.txt',sep='')))[3,]) ,DCCA)
  JIVE=rbind(t(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_JIVE.txt',sep=''))) ,JIVE)
  R.JIVE=rbind(t(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_RJIVE.txt',sep=''))) ,R.JIVE)
  AJIVE=rbind(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_AJIVE.txt',sep='')),AJIVE)
  OnPLS=rbind(as.numeric(t(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_OnPLS.txt',sep='')))),OnPLS)
  DISCO=rbind(t(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_DISCO.txt',sep=''))) , DISCO)
  COBE=rbind(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_COBE.txt',sep='')),COBE)
}
...
max(abs(COBE[,13:14])) #0 

max(abs(R.JIVE[,13]))#8.04105e-13
max(abs(DCCA[,13]))

method='JIVE'
psc=numeric(1000) # proportion of significant correlations between d1 and d2
for(seed in 1:1000){
pvalue=exp(-scan(paste('siml1_seed',seed,'_p900_angle45_nstd1_',method,'_nlogP.txt',sep='')))

result=tryCatch(
  qvalue(pvalue,fdr.level = 0.05), 
  error=function(e){
    qvalue(pvalue,fdr.level = 0.05,pi0=1)
  }
)

psc[seed]=sum(result$significant)/length(pvalue)
}
mean(psc)*100 #in %
sd(psc)*100 #in %
...
for(i in 1:4){
  for(j in 1:4){
    ...
    for(k in 1:1000){
      data[k,]=as.matrix( read.table(paste('siml1_theta_ED_seed',k,'_p',p[i],'_angle',theta[j],'_nstd1.txt',sep='') ))[,1]
    }
...
for(i in 1:4){
  for(j in 1:4){
    data=matrix(0,1000,2)
    for(k in 1:1000){
      data[k,]=as.matrix( read.table(paste('siml1_theta_ED_seed',k,'_p900_angle',theta[j],'_nstd',sigma[i],'.txt',sep='') ))[,1]
    }
...
GDFM.45=NULL
for(seed in 1:1000){
  GDFM.0=rbind(read.table(paste('siml1_seed',seed,'_p900_angle0_nstd1_gdfm.txt',sep='')),GDFM.0)
  GDFM.45=rbind(read.table(paste('siml1_seed',seed,'_p900_angle45_nstd1_gdfm.txt',sep='')),GDFM.45)
...
GDFM.0=as.matrix(GDFM.0)
GDFM.45=as.matrix(GDFM.45)

round(colMeans(GDFM.0)[c(7:10,3:6,15:18)],3)
round(colSds(GDFM.0)[c(7:10,3:6,15:18)],3)

round(colMeans(GDFM.45)[c(7:10,3:6,15:18)],3)
round(colSds(GDFM.45)[c(7:10,3:6,15:18)],3)

## test zero correlations
psc.0.1=numeric(1000) # proportion of significant correlations between d1 and d2
psc.45.1=numeric(1000)
psc.0.2=numeric(1000)
psc.45.2=numeric(1000)
for(seed in 1:1000){
  #weekly common
  pvalue=exp(-scan(paste('siml1_seed',seed,'_p900_angle0_nstd1_gdfm_nlogP_1.txt',sep='')))
  result=tryCatch(
    qvalue(pvalue,fdr.level = 0.05), 
    error=function(e){
      qvalue(pvalue,fdr.level = 0.05,pi0=1)
  ...
  psc.0.1[seed]=sum(result$significant)/length(pvalue)
  
  
  pvalue=exp(-scan(paste('siml1_seed',seed,'_p900_angle45_nstd1_gdfm_nlogP_1.txt',sep='')))
  result=tryCatch(
    qvalue(pvalue,fdr.level = 0.05), 
    error=function(e){
      qvalue(pvalue,fdr.level = 0.05,pi0=1)
  ...
  psc.45.1[seed]=sum(result$significant)/length(pvalue)
  
  #weekly common + weekly idiosyncratic
  pvalue=exp(-scan(paste('siml1_seed',seed,'_p900_angle0_nstd1_gdfm_nlogP_2.txt',sep='')))
  result=tryCatch(
    qvalue(pvalue,fdr.level = 0.05), 
    error=function(e){
      qvalue(pvalue,fdr.level = 0.05,pi0=1)
  ...
  psc.0.2[seed]=sum(result$significant)/length(pvalue)
  
  
  pvalue=exp(-scan(paste('siml1_seed',seed,'_p900_angle45_nstd1_gdfm_nlogP_2.txt',sep='')))
  result=tryCatch(
    qvalue(pvalue,fdr.level = 0.05), 
    error=function(e){
      qvalue(pvalue,fdr.level = 0.05,pi0=1)
  ...
  psc.45.2[seed]=sum(result$significant)/length(pvalue)
