...
%                         of Draftman's plot of 2-d projections
%                         e.g. '.' (default), 'o', '+', 'x'
...
%                         Will add .ps, and save as color postscript 
%
% Outputs:
...
if  nargout == 0  | ...
  ...
  if  ~(idatovlay == 0)   &  imptyp >= 0  ;    %  then add data to plot
    ...
    hold on ;
      ...
      if isstr(dolcolor) && length(markerstr) == 1 ;    %  then can plot colors and markers individually
        if ibigdot == 1 ;   %  plot deliberately large dots
          plot(datatrunc,hts,[dolcolor markerstr],'MarkerSize',1,'LineWidth',2) ;
        else ;    %  use matlab default dots
          plot(datatrunc,hts,[dolcolor markerstr]) ;
        end ;
      elseif length(markerstr) == 1 ;    %  then can plot colors individually
        for idol = 1:length(datatrunc) ;
          if ibigdot == 1 ;   %  plot deliberately large dots
            plot(datatrunc(idol),hts(idol,:),markerstr,...
                  'MarkerSize',1,'LineWidth',2,'Color',dolcolor(idol,:)) ;
          else ;    %  use matlab default dots
            plot(datatrunc(idol),hts(idol),markerstr,...
                  'Color',dolcolor(idol,:)) ;
          end ;
        end ;
      elseif isstr(dolcolor) == 1 ;    %  then can plot markers individually
        for idol = 1:length(datatrunc) ;
          if ibigdot == 1 ;   %  plot deliberately large dots
            plot(datatrunc(idol),hts(idol,:),markerstr(idol),...
                  'MarkerSize',1,'LineWidth',2,'Color',dolcolor) ;
          else ;    %  use matlab default dots
            plot(datatrunc(idol),hts(idol),markerstr(idol),...
                  'Color',dolcolor) ;
          end ;
