...
setwd('/Users/hshu/Desktop/revision2/simulation/setup1_paper_revision2/result_AR1')
###############################################
#####plot1
...
for(j in 1:6){
  for(i in 1:1000){
    data=as.matrix( read.table(paste('siml1_AR1_seed',i,'_p',p[j],'_angle45_nstd1.txt',sep='') ))[,1:12]
    rlt1[i,]=data[1,]
    rlt2[i,]=data[3,]
    rlt3[i,]=data[7,]
  }
...
for(j in 1:6){
  for(i in 1:1000){
    data=as.matrix( read.table(paste('siml1_AR1_seed',i,'_p900_angle',theta[j],'_nstd1.txt',sep='') ))[,1:12]
    rlt1[i,]=data[1,]
    rlt2[i,]=data[3,]
    rlt3[i,]=data[7,]
  }
...
for(j in 1:6){
  for(i in 1:1000){
    data=as.matrix( read.table(paste('siml1_AR1_seed',i,'_p900_angle45_nstd',sigma[j],'.txt',sep='') ))[,1:12]
    rlt1[i,]=data[1,]
    rlt2[i,]=data[3,]
    rlt3[i,]=data[7,]
  }
