...
setwd('/Volumes/My Passport/Hai_Windows/work/Hai_paper1/simulation/setup2_paper_revision1/result')
###############################################
#####plot1
...
for(j in 1:6){
  for(i in 1:1000){
    data=as.matrix( read.table(paste('siml2_seed',i,'_p',p[j],'_angle45_nstd1.txt',sep='') ))[,1:12]
    rlt1[i,]=data[1,]
    rlt2[i,]=data[3,]
    rlt3[i,]=data[7,]
  }
...
for(j in 1:6){
  for(i in 1:1000){
    data=as.matrix( read.table(paste('siml2_seed',i,'_p900_angle',theta[j],'_nstd1.txt',sep='') ))[,1:12]
    rlt1[i,]=data[1,]
    rlt2[i,]=data[3,]
    rlt3[i,]=data[7,]
  }
...
for(j in 1:6){
  for(i in 1:1000){
    data=as.matrix( read.table(paste('siml2_seed',i,'_p900_angle45_nstd',sigma[j],'.txt',sep='') ))[,1:12]
    rlt1[i,]=data[1,]
    rlt2[i,]=data[3,]
    rlt3[i,]=data[7,]
  }
...
for(j in 1:6){
  for(i in 1:1000){
    data1=as.matrix( read.table(paste('siml2_seed',i,'_p',p[j],'_angle45_nstd1.txt',sep='') ))[,1:12]
    data2=as.matrix( read.table(paste('siml2_seed',i,'_p900_angle',theta[j],'_nstd1.txt',sep='') ))[,1:12]
    data3=as.matrix( read.table(paste('siml2_seed',i,'_p900_angle45_nstd',sigma[j],'.txt',sep='') ))[,1:12]
...
sum(rlt[,1]==3)/18000 #17926,  0.9958889
sum(rlt[,2]==5)/18000 #17997,  0.9998333
sum(rlt[,3]==1)/18000 #18000,  1

#summary together with setup 1
(17926+17997+18000+17921+17941+17975)/(18000*6) #0.9977778

(17926+17997+17921+17941)/(18000*4) #0.9970139

(18000+17975)/(18000*2) #0.9993056
